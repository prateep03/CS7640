...
eta         = readParam(options, 'eta', .1);
gamma       = readParam(options, 'gamma', 1/sqrt(2));
radius      = readParam(options, 'radius', 10);
center      = readParam(options, 'center', [0 0]);
center1     = readParam(options, 'center1', [0 0]);
w           = readParam(options, 'tube_width', 0.06);
nb_points   = readParam(options, 'nb_points', 9);
scaling     = readParam(options, 'scaling', 1);
theta       = readParam(options, 'theta', 30 * 2*pi/360);
eccentricity = readParam(options, 'eccentricity', 1.3);
sigma = readParam(options, 'sigma', 0);
...
switch lower(type)
    ...
    case 'bump'
        ...
        s = readParam(options, 'bump_size', .5);
        c = readParam(options, 'center', [0 0]);
        if length(s)==1
            s = [s s];
        end
        x = linspace(-1,1,n);
    ...
    case 'grid-circles'
        ...
        f = readParam(options, 'frequency', 30);
        eta = readParam(options, 'width', .3);
    ...
    case 'chessboard'
        width = readParam(options, 'width', round(n/16) );
    ...
    case 'geometrical'
        ...
        J = readParam(options, 'Jgeometrical', 4);
        sgeom = 100*n/256;
        options.bound = 'per';
        A = ones(n);
        for j=0:J-1
    ...
    case 'lic-texture'
        ...
        options.sigma_tensor = readParam(options, 'lic_regularity', 50*n/256);
        ...
        % initial texture
        lic_width = readParam(options, 'lic_width', 0);
    ...
    case 'line-windowed'
        ...
        [Y,X] = meshgrid(x,x);
        eta = .3; 
        gamma = readParam(options, 'gamma', pi/10);
        parabola = readParam(options, 'parabola', 0);
    ...
    case 'grating'
        x = linspace(-1,1,n);
        [Y,X] = meshgrid(x,x);
        theta = readParam(options, 'theta', .2);
        freq = readParam(options, 'freq', .2);
        X = cos(theta)*X + sin(theta)*Y;
        M = sin(2*pi*X/freq);
    ...
    case 'parabola'
        ...
        c = readParam(c, 'c', .1);
        % angle
        theta = readParam(options, 'theta',  pi/sqrt(2));
        x = -0.5:1/(n-1):0.5;
        [Y,X] = meshgrid(x,x);
    ...
    case 'periodic_bumps'
        nbr_periods = readParam(options, 'nbr_periods', 8);
        theta = readParam(options, 'theta', 1/sqrt(2));
        skew = readParam(options, 'skew', 1/sqrt(2) );        
    ...
    case 'noise'
        sigma = readParam(options, 'sigma', 1);
        M = randn(n) * sigma;
